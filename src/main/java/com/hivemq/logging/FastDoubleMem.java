...
final class FastDoubleMem {
    ...
    private static double dealWithNaN(double value, long str, int startIndex, int endIndex, boolean rejectOverflow) throws NumericException {
        if (Double.isNaN(value)) {
            ...
            byte[] b = new byte[len];
            for (int i = 0; i < len; i++) {
                b[i] = Unsafe.UNSAFE.getByte(str + startIndex + i);
            }
    ...
    private static double parseDecFloatLiteral(
        ...
        for (; index < endIndex; index++) {
            ch = Unsafe.UNSAFE.getByte(str + index);
            if (isDigit(ch)) {
        ...
        int expNumber = 0;
        if (ch == 'e' || ch == 'E') {
            ch = ++index < endIndex ? Unsafe.UNSAFE.getByte(str + index) : 0;
            boolean neg_exp = ch == '-';
            if (neg_exp || ch == '+') {
                ch = ++index < endIndex ? Unsafe.UNSAFE.getByte(str + index) : 0;
            }
    ...
    private static double parseInfinity(long str, int index, int endIndex, boolean negative) throws NumericException {
        if (index + 7 < endIndex
                && Unsafe.UNSAFE.getByte(str + index) == 'I'
                && Unsafe.UNSAFE.getByte(str + index + 1) == 'n'
                && Unsafe.UNSAFE.getByte(str + index + 2) == 'f'
                && Unsafe.UNSAFE.getByte(str + index + 3) == 'i'
                && Unsafe.UNSAFE.getByte(str + index + 4) == 'n'
                && Unsafe.UNSAFE.getByte(str + index + 5) == 'i'
                && Unsafe.UNSAFE.getByte(str + index + 6) == 't'
                && Unsafe.UNSAFE.getByte(str + index + 7) == 'y'
        ) {
    ...
    private static double parseNaN(long str, int index, int endIndex) throws NumericException {
        if (index + 2 < endIndex
                // && str[index] == 'N'
                && Unsafe.UNSAFE.getByte(str + index + 1) == 'a'
                && Unsafe.UNSAFE.getByte(str + index + 2) == 'N') {
    ...
    private static int skipWhitespace(long str, int index, int endIndex) {
        for (; index < endIndex; index++) {
            if ((Unsafe.UNSAFE.getByte(str + index) & 0xff) > ' ') {
                break;
            }
        }
        return index;
    ...
    private static int tryToParseEightDigits(long str, int offset) {
        return FastDoubleSwar.tryToParseEightDigitsUtf8(Unsafe.UNSAFE.getLong(str + offset));
    }
    ...
    static double parseFloatingPointLiteral(long str, int offset, int length, boolean rejectOverflow) throws NumericException {
        ...
        byte ch = Unsafe.UNSAFE.getByte(str + index);
