...
public final class Utf8s {
     ...
     * @return a negative integer, zero, or a positive integer as the left sequence is less than, equal to, or greater
    ...
    public static boolean equalsAscii(
            @NotNull CharSequence lAsciiSeq,
            int lLo,
            int lHi,
            @NotNull Utf8Sequence rSeq,
            int rLo,
            int rHi) {
        int ll = lHi - lLo;
        if (ll != rHi - rLo) {
    ...
    public static boolean equalsIgnoreCaseAscii(
            @NotNull Utf8Sequence lSeq,
            int lLo,
            int lHi,
            @NotNull Utf8Sequence rSeq,
            int rLo,
    ...
    public static int indexOfAscii(
            @NotNull Utf8Sequence seq,
            int seqLo,
            int seqHi,
            @NotNull CharSequence asciiTerm,
            int occurrence) {
        int termLen = asciiTerm.length();
        if (termLen == 0) {
            return -1;
    ...
    public static int indexOfLowerCaseAscii(
            @NotNull Utf8Sequence seq,
            int seqLo,
            int seqHi,
            @NotNull Utf8Sequence termLC) {
    ...
    public static boolean isAscii(long ptr, int size) {
        long i = 0;
        for (; i + 7 < size; i += 8) {
            if ((Unsafe.UNSAFE.getLong(ptr + i) & ASCII_MASK) != 0) {
                return false;
            }
        }
        for (; i < size; i++) {
            if (Unsafe.UNSAFE.getByte(ptr + i) < 0) {
                return false;
            }
        }
        return true;
    ...
    public static boolean startsWith(@NotNull Utf8Sequence seq, @NotNull Utf8Sequence startsWith) {
        ...
        return startsWithSize == 0 ||
                seq.size() >= startsWithSize &&
                        equalPrefixBytes(seq,
                                seq.zeroPaddedSixPrefix(),
                                startsWith,
                                startsWith.zeroPaddedSixPrefix(),
                                startsWithSize);
    }
    ...
    public static void strCpy(long srcLo, long srcHi, @NotNull Utf8Sink dest) {
        for (long i = srcLo; i < srcHi; i++) {
            dest.putAny(Unsafe.UNSAFE.getByte(i));
        }
    }
    ...
    public static void strCpyAscii(char @NotNull [] srcChars, int srcLo, int srcLen, long destAddr) {
        for (int i = 0; i < srcLen; i++) {
            Unsafe.UNSAFE.putByte(destAddr + i, (byte) srcChars[i + srcLo]);
        }
    }
    ...
    public static void utf8ToUtf16Unchecked(
            @NotNull DirectUtf8Sequence utf8CharSeq,
            @NotNull MutableUtf16Sink tempSink) {
        tempSink.clear();
