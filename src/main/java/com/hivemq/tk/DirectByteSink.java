...
public class DirectByteSink implements DirectByteSequence, BorrowableAsNativeByteSink, QuietCloseable, Mutable {
    ...
    public boolean isAscii() {
        return Unsafe.UNSAFE.getByte(impl + BYTE_SINK_ASCII_OFFSET) != 0;
    }
    ...
    public DirectByteSink put(ByteSequence bs) {
        if (bs != null) {
            ...
            for (int i = 0; i < bsSize; i++) {
                Unsafe.UNSAFE.putByte(dest + i, bs.byteAt(i));
            }
            advance(bsSize);
        }
        return this;
...
//        Vect.memcpy(dest, lo, len);
    ...
    public void setAscii(boolean ascii) {
        Unsafe.UNSAFE.putByte(impl + BYTE_SINK_ASCII_OFFSET, (byte) (ascii ? 1 : 0));
    }
    ...
    private long getImplHi() {
        assert impl != 0;
        return Unsafe.UNSAFE.getLong(impl + BYTE_SINK_HI_OFFSET);
    }

    private long getImplLo() {
        assert impl != 0;
        return Unsafe.UNSAFE.getLong(impl + BYTE_SINK_LO_OFFSET);
    }

    private boolean getImplOverflow() {
        return Unsafe.UNSAFE.getByte(impl + BYTE_SINK_OVERFLOW_OFFSET) != 0;
    }

    private long getImplPtr() {
        return Unsafe.UNSAFE.getLong(impl + BYTE_SINK_PTR_OFFSET);
    }
