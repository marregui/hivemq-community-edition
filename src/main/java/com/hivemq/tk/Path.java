...
public class Path implements Utf8Sink, DirectUtf8Sequence, Closeable {
    ...
    public static final ThreadLocal<Path> PATH = ThreadLocal.withInitial(() -> new Path());
    public static final ThreadLocal<Path> PATH2 = ThreadLocal.withInitial(() -> new Path());
    ...
    public Path parent() {
        if (tailPtr > headPtr) {
            ...
            if (last == SEPARATOR || last == NULL) {
                if (p < headPtr + 2) {
                    return this;
                }
                p--;
            }
            while (p > headPtr && Unsafe.UNSAFE.getByte(p) != SEPARATOR) {
                p--;
            }
            tailPtr = p;
        }
    ...
    public Path putNonAscii(long lo, long hi) {
        ascii = false;
        final int size = Bytes.checkAddressingOverflow(lo, hi, this.size());
        checkExtend(size);
//        Vect.memcpy(tailPtr, lo, size);
    ...
    protected final void ensureSeparator() {
        if (tailPtr > headPtr && Unsafe.UNSAFE.getByte(tailPtr - 1) != SEPARATOR) {
            putByte0((byte) SEPARATOR);
        }
    }
