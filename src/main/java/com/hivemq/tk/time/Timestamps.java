...
public final class Timestamps {
    ...
    public static long toMicros(final long value, final @NotNull ChronoUnit unit) {
        switch (unit) {
            case NANOS:
                return value / 1_000;
            case MICROS:
    ...
    /**
     * Since ISO weeks don't always start on the first day of the year, there is an offset of days from the 1st day of
     * the year.
     *
    ...
    public static long toTimezone(long utc, DateLocale locale, CharSequence timezone, int lo, int hi)
            throws NumericException {
        final long offset;
        long l = parseOffset(timezone, lo, hi);
        if (l == Long.MIN_VALUE) {
            return utc +
                    locale.getZoneRules(Numbers.decodeLowInt(locale.matchZone(timezone, lo, hi)), RESOLUTION_MICROS)
                            .getOffset(utc);
        }
    ...
    public static long toUTC(long timestampWithTimezone, DateLocale locale, CharSequence timezone)
            throws NumericException {
        return toUTC(timestampWithTimezone, locale, timezone, 0, timezone.length());
    }

    public static long toUTC(long timestampWithTimezone, DateLocale locale, CharSequence timezone, int lo, int hi)
            throws NumericException {
        long offset;
        long l = parseOffset(timezone, lo, hi);
        if (l == Long.MIN_VALUE) {
            TimeZoneRules zoneRules =
                    locale.getZoneRules(Numbers.decodeLowInt(locale.matchZone(timezone, lo, hi)), RESOLUTION_MICROS);
