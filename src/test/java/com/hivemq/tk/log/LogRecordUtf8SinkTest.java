...
import static com.hivemq.tk.Files.EOL_LENGTH;
...
public class LogRecordUtf8SinkTest {
    ...
    public void testConvoluted() throws Exception {
            ...
            final long buffPtr = Unsafe.malloc(buffSize);
            try {
    ...
    public void testMalformedUtf8Seq() {
        ...
        for (int bufIndex = 0; bufIndex < buffers.length; bufIndex++) {
            ...
            final long msgPtr = Unsafe.malloc(len + EOL_LENGTH);
            try {
    ...
    public void testUtf8LineTrimming() throws Exception {
        runTestUtf8LineTrimmingImpl((msg, utf8ByteLen, sinkMaxLen, expectedLen) -> {
            final long msgPtr = Unsafe.malloc(utf8ByteLen + EOL_LENGTH);
            try {
    ...
    static final class Utf8 implements Closeable {
        ...
        public Utf8(String msg) {
            ...
            lo = Unsafe.malloc(buf.length);
            hi = lo + buf.length;
            Unsafe.UNSAFE.copyMemory(buf, Unsafe.BYTE_OFFSET, null, lo, buf.length);
        }
